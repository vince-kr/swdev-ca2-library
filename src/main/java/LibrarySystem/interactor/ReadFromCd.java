...
public class ReadFromCd extends Interaction{
    ...
    public void requestAndResponse(Library library) {
        ...
        sb.append(StringFormat.fixedLength("DIRECTOR",15));
        sb.append(StringFormat.fixedLength("PLAYTIME",15));
        sb.append(StringFormat.fixedLength("QUANTITY"+RESET,15));
        sb.append("\n");
        sb.append("==================================================================================\n");
        if (!cds.isEmpty()){
            for (Map.Entry<Integer,CdDvd> cd: cds.entrySet()){
                sb.append(StringFormat.fixedLength(cd.getKey(),15));
                sb.append(StringFormat.fixedLength(cd.getValue().getTitle(),15));
                sb.append(StringFormat.fixedLength(cd.getValue().getProducer().getName(),15));
                sb.append(StringFormat.fixedLength(cd.getValue().getProductionYear(),15));
                sb.append(StringFormat.fixedLength(cd.getValue().getDirector().getName(),15));
                sb.append(StringFormat.fixedLength(cd.getValue().getPlayTime(),15));
                sb.append(StringFormat.fixedLength(cd.getValue().getQuantity(),15));
